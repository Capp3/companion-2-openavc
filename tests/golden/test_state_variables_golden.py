"""Golden snapshots for state variable extraction."""
...
def _snapshot_payload(root: Path) -> dict[str, Any]:
    section, review = extract_state_variables(parse_module(root))
    return {
        "state_variables": section.model_dump(exclude_none=True),
        "review": [flag.model_dump() for flag in review],
    }


def test_dummy_state_variables_golden(dummy_device: Path, snapshot: Any) -> None:
    assert _snapshot_payload(dummy_device) == snapshot


def test_bmd_webpresenter_state_variables_golden(
